...
export function useBridge(): AppBridge<TailwindProtocol> {
  return useRawBridge() as AppBridge<TailwindProtocol>;
}
...
export function useApi(): ClientApi {
  const api = useRawApi();
  if (!api) {
    throw new Error('[u-devtools] API not available in tailwind context');
  }
  return api;
}
