...
export function useBridge(): AppBridge<NetworkProtocol> {
  return useRawBridge() as AppBridge<NetworkProtocol>;
}
...
export function useApi(): ClientApi {
  const api = useRawApi();
  if (!api) {
    throw new Error('[u-devtools] API not available in network context');
  }
  return api;
}
