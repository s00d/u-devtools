...
export function useBridge(): AppBridge<LibraryProtocol> {
  return useRawBridge() as AppBridge<LibraryProtocol>;
}
...
export function useApi(): ClientApi {
  const api = useRawApi();
  if (!api) {
    throw new Error('[u-devtools] API not available in library context');
  }
  return api;
}
