...
<template>
  <div class="h-full flex flex-col bg-udt-c-bg text-udt-c-text">
    <!-- Error Banner -->
    <div v-if="error" class="p-4 bg-red-900/20 border-b border-red-800">
      ...
      <div class="text-xs text-red-400 mt-2">
        Add this to your main.ts: <code class="bg-red-900/30 px-1 rounded">window.__U_DEVTOOLS_VUE_ROUTER__ = router</code>
      </div>
    </div>

    <!-- Current Route Info -->
    <div v-if="currentRoute" class="p-4 bg-green-900/20 border-b border-green-800">
      <div class="text-sm font-semibold text-green-200 mb-1">Current Route</div>
      <div class="font-mono text-sm text-green-300">{{ currentRoute.path }}</div>
      <div v-if="currentRoute.name" class="text-xs text-green-400 mt-1">Name: {{ currentRoute.name }}</div>
      <div v-if="currentRoute.from" class="text-xs text-green-500 mt-1">
    ...
    <div class="flex-1 overflow-auto p-4">
      <div v-if="activeRoutes.length > 0" class="mb-6">
        <h3 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Active Route</h3>
        <div class="bg-green-900/20 border border-green-800 rounded">
          <div
            v-for="route in activeRoutes"
            :key="route.path"
            class="p-3 border-b border-green-800 last:border-0 hover:bg-green-900/30"
          >
            <div class="flex items-center gap-2">
              <UBadge color="green">Active</UBadge>
              <span class="font-mono text-sm font-bold text-green-200">{{ route.path }}</span>
              <span v-if="route.name" class="text-xs text-green-400">({{ route.name }})</span>
            </div>
            <div v-if="route.meta && Object.keys(route.meta).length > 0" class="mt-2 text-xs text-gray-400">
      ...
      <div v-if="inactiveRoutes.length > 0">
        ...
        >
          ...
          <template #cell-name="{ val }">
            <span v-if="val" class="text-sm text-gray-400">{{ val }}</span>
