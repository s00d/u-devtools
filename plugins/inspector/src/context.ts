...
export function useBridge(): AppBridge<InspectorProtocol> {
  return useRawBridge() as AppBridge<InspectorProtocol>;
}
...
export function useApi(): ClientApi {
  const api = useRawApi();
  if (!api) {
    throw new Error('[u-devtools] API not available in inspector context');
  }
  return api;
}
