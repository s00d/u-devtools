...
// terminal плагин не использует bridge для событий (только RPC), но для единообразия создаем хук
...
export function useApi(): ClientApi {
  const api = useRawApi();
  if (!api) {
    throw new Error('[u-devtools] API not available in terminal context');
  }
  return api;
}
