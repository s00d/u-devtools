...
export function useBridge(): AppBridge<StorageProtocol> {
  return useRawBridge() as AppBridge<StorageProtocol>;
}
...
export function useApi(): ClientApi {
  const api = useRawApi();
  if (!api) {
    throw new Error('[u-devtools] API not available in storage context');
  }
  return api;
}
