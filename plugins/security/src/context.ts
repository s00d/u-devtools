...
export function useBridge(): AppBridge<SecurityProtocol> {
  return useRawBridge() as AppBridge<SecurityProtocol>;
}
...
export function useApi(): ClientApi {
  const api = useRawApi();
  if (!api) {
    throw new Error('[u-devtools] API not available in security context');
  }
  return api;
}
