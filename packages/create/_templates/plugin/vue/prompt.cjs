...
// If skipping prompts, set the packageName in env so templates can access it
if (skipPrompts && finalPackageName && !packageNameArg) {
  process.env.HYGEN_packageName = finalPackageName;
}

module.exports = skipPrompts
  ? []
  : [
      {
        type: 'input',
      ...
      },
      {
        ...
        name: 'description',
        message: 'Description:',
        default: descriptionArg || 'A plugin for Universal DevTools',
      },
      {
        type: 'multiselect',
        name: 'features',
